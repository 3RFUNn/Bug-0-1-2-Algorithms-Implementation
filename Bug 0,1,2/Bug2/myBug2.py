...
# Initialize sensors with a common sampling period
...
for time_step in range(NUM_TIMESTEPS):
    ...
    if calculate_distance(current_position[0], current_position[1], TARGET_X, TARGET_Y) > 0.1:
        if not OBSTACLE_DETECTED:
            current_direction = 'towards_line'
            # Navigate towards goal until an obstacle is encountered
            motor_speeds = navigate_towards_goal(current_position[0], TARGET_X, current_position[1], TARGET_Y, current_orientation)
            if has_front_wall:
                print('Front wall detected')
                OBSTACLE_DETECTED = True
        else:
            if OBSTACLE_THRESHOLD < 0.2 :
                TURN_RIGHT = True
            ...
            if has_clear_path(current_orientation, current_position[1], TARGET_Y) and previous_direction == 'wall_following' and is_vertical_aligned(current_position[0]):
                OBSTACLE_DETECTED = False
